...
const panic = [
  `肾上腺素激增：之后 2d10={2d10} 分钟内所有投骰带有 [+]，压力减少 1d5。`,
  `紧张：获得 1 压力。`,
  `神经质：获得 1 压力，所有近距乘组获得 2 压力。`,
  `不知所措：之后 1d10={1d10} 分钟内所有投骰带有 [-]，压力下限提升 1。`,
  `懦弱：获得一项新状态：必须通过一次恐惧豁免才能参与暴力行为，否则就会逃跑。`,
  `受惊：获得一项新状态：恐惧症：在遭遇恐惧源时得要通过一次恐惧豁免 [-]，否则获得 1d5={1d5} 压力。`,
  ...
  `紧张症：2d10={2d10} 分钟里变得毫无反应，呆若木鸡。减少 1d10={1d10} 压力。`,
  `狂暴：之后 1d10={1d10} 小时内，所有伤害投骰带有 [+]。所有乘组获得 1 压力。`,
  `沦陷：获得一项新状态：进行一次带有 [-] 的惊恐检定。`,
  `复合问题：按照此惊恐表投骰两次。压力下限提升 1。`,
  `心脏病发/短路（仿生人）：损伤上限降低 1。1d10={1d10} 小时内所有投骰带有 [-]。压力下限提升 1。`,
  `退场：投个新角色来参加游戏。`
]
...
function handleStress(mctx, isFailure, isRest, dice) {
  ...
  let exceed = parseInt(seal.format(mctx, "{压力溢出}"));
  const stressMin = parseInt(seal.format(mctx, "{压力下限}"));
  const stressMax = parseInt(seal.format(mctx, "{压力上限}"));
  ...
  if (cStress < stressMax) {
    ...
    exceed += 1;
    seal.vars.intSet(mctx, "压力溢出", exceed);
    stressChangeText = `\n压力：20→20 (已达上限，溢出值${exceed}点，将自动转化为检定减值)`;
    return stressChangeText;
  }
}
...
function drawPanic(mctx,idx){
  let text = `\n\n惊恐效果:\n`;
  if (idx != 17){
    text += seal.format(mctx,panic[idx]);
    return text;
  }else{
    text += seal.format(mctx,panic[idx]);
    text += `\n复合惊恐效果：\n`
    for (i=0; i<2; i++){
      ...
      text += seal.format(mctx,panic[die]);
      text += "\n";
    }
  }
  return text;
...
const solveCheck = (ctx, msg, cmdArgs, mode) => {
  ...
  let check = 0;
  let isRest = false;
  const cStress = parseInt(seal.format(mctx, "{压力}"));
  const penalty = (cStress < 20)? 0: parseInt(seal.format(mctx, "{压力溢出}"));
  ...
  if (val === "死亡") {
    ...
    if (roll == 0){
      replytext += seal.format(mctx,`你失去意识。你会在 2d10={2d10} 分钟内醒来，生命值上限永久-1d5={1d5}。`);
    }else if(roll <= 2){
      replytext += seal.format(mctx,`你失去意识，生命垂危。若是不加干预，你会在 1d5={1d5} 轮内死亡。`);
    }else if(roll<=4){
      replytext += seal.format(mctx,`你失去意识，生命垂危。若是不加干预，你会在 1d5={1d5} 轮内死亡。`);
    }else {
